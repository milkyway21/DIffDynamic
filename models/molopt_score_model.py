...
class ScorePosNet3D(nn.Module):  # 定义三维位置-类别扩散模型。
    ...
    def _prepare_ligand_inputs(self, init_ligand_v):  # 预处理配体类别输入，统一为张量形式。
        if torch.is_floating_point(init_ligand_v):  # 若输入已是浮点表示。
            if init_ligand_v.dim() != 2:  # 检查维度是否符合 [N, num_classes]。
                raise ValueError('Expect ligand features of shape [N, num_classes] when providing floating tensors.')  # 抛出异常提示。
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
        ...
        final_ligand_pos, final_ligand_h = final_pos[mask_ligand], final_h[mask_ligand]  # 选择配体对应的结果。
    ...
    def _predict_x0_from_eps(self, xt, eps, t, batch):  # 根据噪声估计还原 x0。
        pos0_from_e = extract(self.sqrt_recip_alphas_cumprod, t, batch) * xt - \
                      extract(self.sqrt_recipm1_alphas_cumprod, t, batch) * eps  # 按扩散公式反推 x0。
    ...
    def q_pos_posterior(self, x0, xt, t, batch):  # 计算连续变量后验均值。
        ...
        pos_model_mean = extract(self.posterior_mean_c0_coef, t, batch) * x0 + \
                         extract(self.posterior_mean_ct_coef, t, batch) * xt  # 根据闭式公式计算均值。
    ...
    def _dynamic_diffusion(self, protein_pos, protein_v, batch_protein,
                           ...
                           use_with_noise=False):  # 执行动态扩散迭代（用于采样或精炼），增强数值稳定性和监控。use_with_noise: 是否在类别更新中使用带噪声的方法。
        ...
        for t_scalar in time_indices:  # 遍历时间调度列表。
            ...
            if self.model_mean_type == 'noise':  # 当模型预测噪声时。
                pred_pos_noise = preds['pred_ligand_pos'] - ligand_pos  # 计算预测噪声。
                ...
                pos0_from_e = preds['pred_ligand_pos']  # 直接使用预测。
            else:  # 未知模式。
                raise ValueError  # 抛出异常。
            ...
            ligand_pos = ligand_pos + current_step_size * combined_pos_grad  # 按步长更新配体坐标。
            ...
            if not pos_only:  # 若同时更新类别。
                ...
                if use_adaptive_step:  # 若启用自适应步长。
                    ...
                    current_step_size = adaptive_step_size(combined_v_grad_norm, step_size)  # 自适应调整步长。
                
                log_ligand_v = F.log_softmax(log_ligand_v + current_step_size * combined_v_grad, dim=-1)  # 更新 log 概率。
    ...
    def sample_diffusion_large_step(self, protein_pos, protein_v, batch_protein,
        ...
        if not use_lambda_schedule:  # 若使用固定步长调度。
    ...
    def sample_diffusion_refinement(self, protein_pos, protein_v, batch_protein,
        ...
        if not use_lambda_schedule:  # 固定步长调度。
        ...
        # 根据官方 GlintDM 实现，在精炼采样初始化时使用带噪声的预测
...
class GlintDM(ScorePosNet3D):  # 定义 GlintDM 模型，继承 ScorePosNet3D。

    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):  # 初始化 GlintDM 模型。
    ...
    def dynamic_sample_diffusion(self, protein_pos, protein_v, batch_protein,
        ...
        large_stride = None if large_cfg.get('schedule', 'lambda') == 'lambda' else large_cfg.get('stride')  # 确定大步步长。
        ...
        refine_stride = None if refine_cfg.get('schedule', 'lambda') == 'lambda' else refine_cfg.get('stride')  # 精炼步长。
...
def extract(coef, t, batch):  # 根据时间步从常量表中取出并匹配批次。
    out = coef[t][batch]  # 先按时间索引，再按批次索引提取。
    return out.unsqueeze(-1)  # 扩展最后一维以便广播。
