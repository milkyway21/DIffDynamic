...
def compose_context(h_protein, h_ligand, pos_protein, pos_ligand, batch_protein, batch_ligand):  # 定义蛋白-配体上下文拼接函数。
    ...
    # sort_idx = batch_ctx.argsort()  # 保留注释：旧的排序实现。
    sort_idx = torch.sort(batch_ctx, stable=True).indices  # 使用稳定排序获取索引，保证类型顺序稳定。

    mask_ligand = torch.cat([  # 构建配体掩码，标记哪些节点属于配体。
        torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),  # 对蛋白节点标记为 False。
        torch.ones([batch_ligand.size(0)], device=batch_ligand.device).bool(),  # 对配体节点标记为 True。
